...
import { MessagesSquare, PanelLeftIcon, PanelRight, Plus, User, Coins, LogOut, PenSquare } from 'lucide-react'
    ...
    <div className='fixed lg:static inset-y-0 left-0 z-50 w-[270px] h-screen shrink-0 bg-[#0d0f14] border-r border-white/[0.06]'>
      <div className='flex flex-col h-full'>
        ...
        {conversations.length === 0 ? (
          ...
          <div className='px-5 pt-4 pb-1.5 text-[10.5px] font-semibold uppercase text-slate-600'>Recents</div>
        )}
          ...
          {userData ? (
            <div className='flex items-center gap-2.5 cursor-pointer rounded-xl px-3 py-2.5 hover:bg-white/[0.05] transition-colors duration-150'>
              ...
              <div className='flex-1 min-w-0'>
                <p className='text-[13.5px] font-semibold text-slate-100 truncate'>{userData?.name || userData?.email || 'user'}</p>
                <p className='text-[11px] text-slate-600 mt-px'>{userData?.email ? userData.email : 'Free Plan'}</p>
              </div>
              <div className='flex gap-1'>
                ...
                <button
                  className='flex items-center justify-center w-7 h-7 rounded-[7px] border-none bg-transparent text-slate-600 cursor-pointer hover:text-slate-400 transition-all duration-150'
            ...
            <button className='w-full flex items-center justify-center gap-2 text-sm font-medium text-slate-200 bg-white/[0.05] border border-white/[0.08] rounded-xl py-[11px] cursor-pointer hover:bg-white/[0.08] transition-colors duration-150'>Login</button>
          )}
        </div>
      </div>
    </div>
